...
impl LogFile {
    ...
    pub fn search_field(&self, conditional_or_none: &Option<QueryClause>) -> Vec<Row> {
        match conditional_or_none {
            &Some(ref conditional) => {
                self.rows.iter()
                    .filter(|r| r.matches(&conditional.conditional_field, &conditional.conditional_value))
                    .map(|r| r.clone())
                    .collect()
            },
            &None => {
